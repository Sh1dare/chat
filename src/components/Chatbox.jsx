            ...
            {selectedUser ? (
                <section className="flex flex-col items-start justify-start h-screen w-[100%] background-image">
                    <header className="w-[100%] h-[82px] m:h-fit p-4 bg-white">
                        <main className="flex items-center gap-3">
                            ...
                            <span>
                                <h3 className="font-semibold text-[#2A3D39] text-lg">{selectedUser?.fullName || "Пользователь Monogram"}</h3>
                                <p className="font-light text-[#2A3D39] text-sm">@{selectedUser?.username || "Monogram"}</p>
                            </span>
                        </main>
                    </header>
                    ...
                    <main className="custom-scrollbar relative h-[100vh] w-[100%] flex flex-col justify-between">
                        ...
                        <div className="sticky lg:bottom-0 bottom-[60px] p-3 h-fit w-[100%]">
                            <form onSubmit={handleSendMessage} action="" className="flex items-center bg-white h-[45px] w-[100%] px-2 rounded-lg relative shadow-lg">
                                <input value={messageText} onChange={(e) => sendMessageText(e.target.value)} className="h-full text-[#2A3D39] outline-none text-[16px] pl-3 pr-[50px] rounded-lg w-[100%]" type="text" placeholder="Написать сообщение..." />
                ...
                <section className="h-screen w-[100%] bg-[#e5f6f3]">
                    <div className="flex flex-col justify-center items-center h-[100vh]">
                        ...
                        <h1 className="text-[30px] font-bold text-teal-700 mt-5">Добро пожаловать в Monogram!</h1>
                        <p className="text-gray-500">Общайтесь с друзьями быстро и надёжно!</p>
                    </div>
                </section>
            )}
        </>
